...
import {
  ...
  completePendingRegistrationWithPassword,
  publicAuthUser,
...
async function sessionResponse(user: { id: string; email: string; username: string; emailVerifiedAt: Date | null; profile?: { fullName?: string | null } | null }) {
  const cookieStore = await cookies();
  const guestToken = cookieStore.get(CART_COOKIE)?.value;
  await mergeGuestCartIntoUser(guestToken, user.id);

  const token = await createSessionToken(user.id);
  const response = NextResponse.json({ user: publicAuthUser(user) });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      const gated = await allowUnverifiedLogin(user, origin);
      if (!gated.allow) {
        return NextResponse.json({ error: "email_unverified", message: UNVERIFIED_MESSAGE }, { status: 403 });
      }
      return sessionResponse(gated.user);
    } catch (error) {
      const completed = await completePendingRegistrationWithPassword(payload, origin).catch(() => null);
      if (completed?.needsEmail) {
        return NextResponse.json({ error: "email_unverified", message: UNVERIFIED_MESSAGE }, { status: 403 });
      }
      if (completed?.user) {
